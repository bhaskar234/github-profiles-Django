...
def repoinfo(request, name, owner):
    ...
    return render_to_response('githubprofiles/index.html',
                              {'template': 'repoinfo',
                               'repoinfo_list': repoinfo_list})


def repos(request, login):
    repos_list = Repos.objects.filter(owner=login)
    return render_to_response('githubprofiles/index.html',
                              {'template': 'repos', 'repos_list': repos_list})
...
def index(request):
    ...
    if("access_token" in request.session):
        ...
        if(request.session['access_token'] != "" and
           request.session['access_token'] != None):
            ...
            msg = ""
            if(n.count() == 0):
                userkeys = {"type", "login", "collaborators", "public_repos",
                            "public_gists", "private_gists", "followers",
                            "gravatar_id", "html_url", "url", "created_at"}
                userdict = {key: value for (key, value) in
                            j.items() if key in userkeys}
                ...
                r = response.read()
                j = json.loads(r)
                repokeys = {"url", "homepage", "watchers", "open_issues",
                            "created_at", "pushed_at", "has_issues", "fork",
                            "has_downloads", "private", "name", "description",
                            "forks", "owner", "has_wiki"}
                for i in range(len(j['repositories'])):
                    repodict = {key: value for (key, value) in
                                j['repositories'][i].items() if
                                key in repokeys}
                    repo = Repos(**repodict)
                    repo.save()
...
def auth(request):
    ...
    getTokenUrl = "https://github.com/login/oauth/access_token?client_id=" +
    "799c118a4784b2e967f8&redirect_uri=" +
    redirecturi + "&client_secret=" + client_secret

    getCodeUrl = "https://github.com/login/oauth/authorize?client_id=" +
    "799c118a4784b2e967f8&redirect_uri=" + redirecturi

    getAccessToken = 0
    if("access_token"  in request.session):
        if(request.session['access_token'] == "" or
           request.session['access_token'] == None):
            getAccessToken = 1
